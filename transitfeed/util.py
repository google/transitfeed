        ...
        for (
            ...
            for local_name, local_val in list(frame_obj.f_locals.items()):
                try:
                    ...
                    dump.append(
                        "    Exception in str(%s): %s" % (local_name, e)
                    )
                else:
...
def _MaxVersion(versions):
    ...
    if len(versions) > 0:
        ...
        raise Exception("Two versions to compare is needed")
...
def ValidateRequiredFieldsAreNotEmpty(
    gtfs_object, required_field_names, problems=None
):
    """
...
def ValidateAndReturnIntValue(
    ...
    try:
        ...
        if int_value not in allowed_values:
            if problems and column_name:
                problems.InvalidValue(
                    column_name, value, type=errors.TYPE_WARNING
                )
        return int_value
