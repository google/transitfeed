  ...
  an agency.  This is the main class for this module."""
    ...
    def __init__(
        ...
        if gtfs_factory is None:
            # This hackery is due to the cyclic dependency mess we currently
            #  have. See gtfsfactoryuser for more.
        ...
        #  there can be more than one transfer with each ID. Once GTFS
        #  explicitly prohibits duplicate IDs this might be changed to a simple
        #  dict of Transfers.
    ...
    def ConnectDb(self, memory_db):
        ...
        if memory_db:
            ...
            try:
                ...
                # Windows won't let a file be opened twice. mkstemp does not
                #  remove the file when all handles to it are closed.
    ...
    def GetDefaultAgency(self):
        """Return the default Agency. If no default Agency has been set select
        the default depending on how many Agency objects are in the Schedule.
        If there are 0 make a new Agency the default, if there is 1 it becomes
        the default, if there is more than 1 then return None."""
    ...
    def NewDefaultAgency(self, **kwargs):
        """Create a new Agency object and make it the default agency for this
        Schedule"""
        ...
        self.SetDefaultAgency(
            agency, validate=False
        )  # Blank agency won't validate
        return agency

    def SetDefaultAgency(self, agency, validate=True):
        """Make agency the default and add it to the schedule if not already
        added"""
    ...
    def GetDefaultServicePeriod(self):
        """Return the default ServicePeriod. If no default ServicePeriod has
        been set select the default depending on how many ServicePeriod objects
        are in the Schedule. If there are 0 make a new ServicePeriod the
        default, if there is 1 it becomes the default, if there is more than 1
        then return None."""
        if not self._default_service_period:
            if len(self.service_periods) == 0:
                ...
                self._default_service_period = list(
                    self.service_periods.values()
                )[0]
        return self._default_service_period

    def NewDefaultServicePeriod(self):
        """Create a new ServicePeriod object, make it the default service
        period and return it. The default service period is used when you
        create a trip without providing an explict service period. """
    ...
    def AddServicePeriodObject(
        ...
        if service_period.service_id in self.service_periods:
            problem_reporter.DuplicateID(
                "service_id", service_period.service_id
            )
            return
    ...
    def GetDateRangeWithOrigins(self):
        """Returns a tuple of (earliest, latest, earliest_origin,
        latest_origin) dates on which the service periods in the schedule
        define service, in YYYYMMDD form. The origins specify where the
        earliest or latest dates come from. In particular, whether the date is
        a regular ServicePeriod start_date or end_date in calendar.txt, a
        service exception of type add in calendar_dates.txt, or feed start/end
        date defined in feed_info.txt."""
        ...
        # Override with feed_info.txt feed_start_date and feed_end_date values,
        # if defined
    ...
    """
        ...
        stop = self._gtfs_factory.Stop(
            stop_id=stop_id, lat=lat, lng=lng, name=name
        )
    ...
    def AddFeedInfoObject(
        self, feed_info, problem_reporter=None, validate=False
    ):
    ...
    def AddTransferObject(self, transfer, problem_reporter=None):
        assert (
            transfer._schedule is None
        ), "only add Transfer to a schedule once"
        if not problem_reporter:
            problem_reporter = self.problem_reporter
    ...
    """
        ...
        if "feed_info" in self._table_columns:
            ...
            self._WriteArchiveString(
                archive, "feed_info.txt", feed_info_string
            )
        ...
        if headway_rows:
            ...
            self._WriteArchiveString(
                archive, "frequencies.txt", headway_string
            )
        ...
        if self.GetFareAttributeList():
            ...
            self._WriteArchiveString(
                archive, "fare_attributes.txt", fare_string
            )
    ...
    """
        ...
        date_services = self.GetServicePeriodsActiveEachDate(
            date_start, date_end
        )
        date_trips = []
        ...
        for date, services in date_services:
            day_trips = sum(
                service_id_to_trips[s.service_id] for s in services
            )
            day_departures = sum(
    ...
    def ValidateAgenciesHaveSameAgencyTimezone(self, problems):
        timezones_set = set(
            [agency.agency_timezone for agency in self.GetAgencyList()]
        )
    ...
    """
        ...
        # We have to check if there is a gap at the end of the specified date
        #  range
        if consecutive_days_without_service >= service_gap_interval:
            problems.TooManyDaysWithoutService(
                first_day_without_service,
    ...
    def ValidateServiceExceptions(
        self, problems, first_service_day, last_service_day
    ):
        ...
        for period in self.GetServicePeriodList():
            # If at least one ServicePeriod has service exceptions we don't
            # issue the warning, so we can stop looking at the list of
            # ServicePeriods.
            if period.HasExceptions():
                return
        problems.NoServiceExceptions(
            start=first_service_day, end=last_service_day
        )

    def ValidateServiceRangeAndExceptions(
        self, problems, today, service_gap_interval
    ):
        if today is None:
        ...
        if not end_date or not start_date:
            ...
            try:
                ...
                # Format of start_date and end_date checked in class
                # ServicePeriod
                pass
    ...
    def ValidateStops(self, problems, validate_children):
        ...
        # parent_station references. Also check that the parent station isn't
        # too far from its child stops.
    ...
    def ValidateNearbyStops(self, problems):
        ...
        # index.
        sorted_stops = [
            s for s in self.GetStopList() if s.stop_lat and s.stop_lon
        ]
        sorted_stops.sort(
            key=(
                lambda x: [x.stop_lat, x.stop_lon, getattr(x, "stop_id", None)]
            )
        )
            ...
            while (index < len(sorted_stops)) and (
                ...
                if distance is not None and distance < 2:
                    ...
                    if (
                        ...
                        and other_stop.location_type == 0
                    ):
                        ...
                        and other_stop.location_type == 1
                    ):
                    ...
                    elif stop.location_type in (
                        0,
                        1,
                    ) and other_stop.location_type in (0, 1):
                        if (
                            stop.location_type == 0
                            and other_stop.location_type == 1
                        ):
    ...
    def ValidateTrips(self, problems):
        stop_types = (
            {}
        )  # a dict mapping stop_id to [route_id, route_type, is_match]
        ...
        for trip in self.trips.values():
            ...
            for index, st in enumerate(stop_times):
                ...
                if (
                    ...
                    or route_type
                    == self._gtfs_factory.Route._ROUTE_TYPE_NAMES["Bus"]
                ):
                    if stop_id not in stop_types:
                        ...
                        if (
                            ...
                            == self._gtfs_factory.Route._ROUTE_TYPE_NAMES[
                                "Subway"
                            ]
                        ):
            ...
            if not util.IsEmpty(trip.block_id) and stop_times:
                ...
                if (
                    first_arrival_secs is not None
                    and last_departure_secs is not None
                ):
    ...
    def ValidateBlocks(self, problems, trip_intervals_by_block_id):
        ...
        for (block_id, trip_intervals) in list(
            trip_intervals_by_block_id.items()
        ):
            ...
            for xi in range(len(trip_intervals)):
                ...
                for xj in range(xi + 1, len(trip_intervals)):
                    ...
                    if trip_a.service_id == trip_b.service_id:
                        ...
                        if (
                            service_id_pair_key
                            not in service_period_overlap_cache
                        ):
                            ...
                            service_period_a = self.GetServicePeriod(
                                trip_a.service_id
                            )
                            service_period_b = self.GetServicePeriod(
                                trip_b.service_id
                            )
    ...
    def ValidateTripStopTimes(self, problems):
        ...
        for trip in list(self.trips.values()):
            ...
            if not count_stop_times:
                ...
                if (
                    len(trip._headways) > 0
                ):  # no stoptimes, but there are headways
                    problems.OtherProblem(
    ...
    def Validate(
       ...
       (mostly interrelationships between the various data sets)."""
        ...
        self.ValidateServiceRangeAndExceptions(
            problems, today, service_gap_interval
        )
