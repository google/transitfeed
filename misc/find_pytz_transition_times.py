...
for tz_name in pytz.common_timezones:
    ...
    for tran, inf in zip(
        tzinfo._utc_transition_times, tzinfo._transition_info
    ):
        if tran < datetime.datetime(2009, 6, 1) or tran > datetime.datetime(
            2010, 9, 1
        ):
        ...
        try:
            ...
            tran_beforelocal = before.astimezone(tzinfo) + datetime.timedelta(
                seconds=1
            )
            tran_afterlocal = after.astimezone(tzinfo) + datetime.timedelta(
                seconds=-1
            )
