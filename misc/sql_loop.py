...
def LoadFile(f, table_name, conn):
    ...
    try:
        c.execute(
            "CREATE TABLE %s (%s)" % (table_name, ",".join(create_columns))
        )
...
def main():
    ...
    parser = optparse.OptionParser(
        usage="usage: %prog [options] [sql command]"
    )
    parser.add_option(
        "-f",
        "--load_file",
    ...
    parser.add_option(
        "", "--db", dest="database", metavar="FILE", help="sqlite db"
    )
