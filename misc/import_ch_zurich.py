...
class DivaImporter:
    def __init__(self, coord_converter, drop_unadvertised_lines):
        ...
        self.pickup_type = (
            {}
        )  # (trip_id, stop_seq) --> '0'=normal/'1'=no pickup
        self.drop_off_type = (
            {}
        )  # (trip_id, stop_seq) --> '0'/'1', '1'=no drop-off
        self.trips = {}  # id --> Trip
        self.goodTrips = {}
    ...
    def Import(self, inpath):
        ...
        self.ImportStations(
            read("rec_ort.mdv"),
            read("bedienendeLinien_google.csv", "ORT_NR;LI_NR;;;;"),
        )
        self.ImportRoutes(read("rec_lin_ber.mdv"))
        self.ImportPatterns(read("lid_verlauf.mdv"))
        self.ImportServices(
            read("tagesart_merkmal.mdv"), read("firmenkalender.mdv")
        )
    ...
    def ImportStations(self, station_file, adv_file):
        ...
        for station_id, line_id in ReadCSV(adv_file, ["ORT_NR", "LI_NR"]):
            if station_id in self.stations:
                ...
                self.stations[station_id].advertised_lines.add(
                    line_id.lstrip("0")
                )
            else:
                print(
    ...
    def ImportServices(self, daytype_file, days_file):
        ...
        for schedule, daytype, date in ReadCSV(
            ...
            daytypes.setdefault("%s.%s" % (schedule, daytype), {})[
                int(date)
            ] = 1
    ...
    def ImportTrafficRestrictions(self, restrictions_file):
        ...
        ParseDate = lambda x: datetime.date(
            int(x[:4]), int(x[4:6]), int(x[6:8])
        )
    ...
    def Write(self, outpath):
        ...
        out = zipfile.ZipFile(
            outpath, mode="w", compression=zipfile.ZIP_DEFLATED
        )
