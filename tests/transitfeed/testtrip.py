...
class DuplicateStopSequenceTestCase(util.TestCase):
    def runTest(self):
        ...
        schedule.Load(
            util.DataPath("duplicate_stop_sequence"), extra_validation=True
        )
...
class MissingEndpointTimesTestCase(util.TestCase):
    def runTest(self):
        ...
        schedule = transitfeed.Schedule(problem_reporter=problems)
        schedule.Load(
            util.DataPath("missing_endpoint_times"), extra_validation=True
        )
...
class TripMemoryZipTestCase(util.MemoryZipTestCase):
    ...
    def testExtraObjectAttribute(self):
        ...
        trip2.AddStopTime(
            stop=schedule.GetStop("BULLFROG"), stop_time="09:00:00"
        )
        trip2.AddStopTime(
            stop=schedule.GetStop("STAGECOACH"), stop_time="09:30:00"
        )
...
class TripHasStopTimeValidationTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        self.assertTrue(
            e.FormatProblem().find("Frequencies defined, but") != -1
        )
        ...
        trip.AddStopTime(
            stop, arrival_time="5:11:00", departure_time="5:12:00"
        )
        self.ValidateAndExpectOtherProblem(schedule)
        ...
        schedule.AddStopObject(stop)
        trip.AddStopTime(
            stop, arrival_time="5:15:00", departure_time="5:16:00"
        )
        schedule.Validate(self.problems)
...
class StopMatchWithShapeTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        schedule.Validate(self.problems)
        e = self.accumulator.PopException(
            "StopTooFarFromShapeWithDistTraveled"
        )
...
class TripClearStopTimesTestCase(util.TestCase):
    def runTest(self):
        ...
        route = schedule.AddRoute(
            short_name="54C", long_name="Hill", route_type=3
        )
...
class AddStopTimeParametersTestCase(util.TestCase):
    def runTest(self):
        ...
        route = schedule.AddRoute(
            short_name="10", long_name="", route_type="Bus"
        )
        ...
        trip.AddStopTime(
            stop, arrival_time="00:07:00", departure_time="00:07:30"
        )
...
class AddFrequencyValidationTestCase(util.ValidationTestCase):
    def ExpectInvalidValue(
        self, start_time, end_time, headway, column_name, value
    ):
        try:
            trip = transitfeed.Trip()
    ...
    def runTest(self):
        ...
        self.ExpectInvalidValue(
            "12:00:00", "06:00:00", 1200, "end_time", 21600
        )
...
class GetTripTimeTestCase(util.TestCase):
    ...
    def setUp(self):
        ...
        self.schedule = schedule
        schedule.AddAgency(
            "Agency", "http://iflyagency.com", "America/Los_Angeles"
        )
        service_period = schedule.GetDefaultServicePeriod()
        service_period.SetDateHasService("20070101")
    ...
    def testGetStopTimeTrips(self):
        ...
        self.assertEqual(
            ("trip1", "trip2", "trip2"),
            tuple([ti[0].trip_id for ti in trip_index]),
        )
...
class GetFrequencyTimesTestCase(util.TestCase):
    ...
    def testGetFrequencyStartTimes(self):
        ...
        self.assertEqual(
            ...
            [
                transitfeed.FormatSecondsSinceMidnight(secs)
                for secs in start_times
            ],
        )
    ...
    def testGetFrequencyStopTimes(self):
        ...
        for stoptimes in stoptimes_list:
            ...
            headway_departure_secs.append(
                [st.departure_secs for st in stoptimes]
            )
            ...
            for st_clone, st in zip(stoptimes_clone, stoptimes):
                for name in st.__slots__:
                    if name not in ("arrival_secs", "departure_secs"):
                        self.assertEqual(
                            getattr(st, name), getattr(st_clone, name)
                        )
